                                    ...
                                    <div class="form-group mx-sm-3 mb-2">
                                        <select name="status" class="form-control">
                                            ...
                                            <option value="confirmed"
                                                {{ request()->input('status') == 'confirmed' ? 'selected' : '' }}>Đã xác
                                                nhận</option>
                                            <option value="pending"
                                                {{ request()->input('status') == 'pending' ? 'selected' : '' }}>Chờ xác nhận
                                            </option>
                                            <option value="canceled"
                                                {{ request()->input('status') == 'canceled' ? 'selected' : '' }}>Đã hủy
                                            </option>
                                        </select>
                                    </div>
                                ...
                                <tbody>
                                    @foreach ($reservations as $reservation)
                                        <tr>
                                            ...
                                            <td>{{ \Carbon\Carbon::parse($reservation->reservation_date)->format('d/m/Y H:i') }}
                                            </td>
                                            {{-- <td>{{ $reservation->guest_count }} khách</td> --}}
                                            ...
                                            <td>
                                                ...
                                                @if ($reservation->status == 'pending')
                                                    <!-- Nút chuyển khoản cho các đơn pending -->
                                                    <a href="{{ route('payment.create', ['reservationId' => $reservation->id, 'amount' => 10000]) }}" class="btn btn-info btn-sm">
